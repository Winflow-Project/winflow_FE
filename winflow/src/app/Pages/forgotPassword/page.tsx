...
// Define the type for the successful response from the mutation
...
const ForgotPassword = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    forgotPasswordMutation.mutate(
      ...
      {
        // Explicitly define the type for the data in the onSuccess callback
        onSuccess: (data: ForgotPasswordSuccessResponse) => {
          const msg = data?.message || "Password reset link sent!";
          toast.success(msg);
          router.push("/Pages/checkMail");
        },
      ...
      <div className="bg-white shadow-md rounded-xl p-8 w-full max-w-md border border-gray-200">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Button
            type="submit"
            disabled={forgotPasswordMutation.isPending}
            className={`w-full mt-6 py-2.5 rounded-md shadow-md font-medium transition ${
